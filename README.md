...
- `output/droneshine_edit.mp4` — 17 s CapCut-style rhythm edit built by
  `scripts/build_edit.py`: photos run through FSRCNN 4x super-resolution
  (`assets/sr/`, model weights gitignored), audio is voiceover + booms/whooshes
  only — no music, cuts sit on a 0.5 s grid so a user-added ~120 bpm track syncs
